...
interface AdminRequiredProps {
	children: ReactNode;
	/** Custom message to show when admin access is required */
	message?: string;
	...
	/** Fallback component when not admin (instead of default UI) */
	fallback?: ReactNode;
}

/**
 * AdminRequired - Wraps content that requires admin privileges
 *
 * Shows the children if the user is logged in AND is an admin.
 * Shows a login prompt if the user is not logged in.
 * Shows an access denied UI if the user is logged in but not an admin.
 *
 * @example
 * ```tsx
 * <AdminRequired message="Only admins can manage users">
 *   <UserManagementPanel />
 * </AdminRequired>
 * ```
 */
export function AdminRequired({
	children,
	message = "You must be an administrator to access this section of TownSpark.",
	dashboardPath = "/",
	onDashboardClick,
	onLogoutClick,
	loadingComponent,
